...
# kill the node after 2 minutes for testing
sleep 600; ps -ef | grep tezos-node | grep -v grep | awk '{print $2}' | xargs kill & \
# start the baker once the node is running
sleep 60; ./tezos-baker-006-PsCARTHA run with local node /data/archive_store /data/blocks_above_933913 > /data/baker_logs 2>&1 && \
